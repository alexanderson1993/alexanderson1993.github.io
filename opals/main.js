...
PaperFunctions.resetCanvas = function(){
	Object.keys(PaperLayers).forEach(function(e){
		localStorage.setItem('savedDesign' + e,'');
	});
	resetCanvas();
};

PaperFunctions.saveCanvas = function(){
	Object.keys(PaperLayers).forEach(function(e){
		localStorage.setItem('savedDesign' + e,PaperLayers[e].exportJSON());
	});
};
...
function resetCanvas(product){
	var productName;
	//Load the corner icons
	PaperLayers.bound.importSVG('svg/rotate.svg',function(a){
		PaperLayers.bound.activate();
		a.name = 'rotate';
		rotate = new Symbol(a);
	});
	PaperLayers.bound.importSVG('svg/move.svg',function(a){
		PaperLayers.bound.activate();
		a.name = 'scale';
		scale = new Symbol(a);
	});
	PaperLayers.bound.importSVG('svg/trash.svg',function(a){
		PaperLayers.bound.activate();
		a.name = 'trash';
		trash = new Symbol(a);
	});
	if (localStorage.getItem('savedDesignbackground')){
		//There is a saved design. Return at the end.
		Object.keys(PaperLayers).forEach(function(e){
			PaperLayers[e].importJSON(localStorage.getItem('savedDesign' + e));
		});
		return;
	}
	//Remove all of the items on the canvas.
	PaperLayers.background.removeChildren();
	PaperLayers.export.removeChildren();
	PaperLayers.template.removeChildren();
	PaperLayers.product.removeChildren();
	PaperLayers.content.removeChildren();
	PaperLayers.overlay.removeChildren();
	PaperLayers.bound.removeChildren();

	if (!product) productName = 'Grip';
	else productName = product;
	project.view.resolution = 300;
	PaperLayers.background.activate();
	backgroundRect();
	PaperLayers.overlay.importSVG('GripTemplate.svg',function(a){
		PaperItems.gripTemplate = a;
		PaperLayers.overlay.addChild(a);
		PaperItems.gripTemplate.scale(0.21);
		PaperItems.gripTemplate.position.setX(PaperItems.gripTemplate.bounds.width/2 + 169);
		PaperItems.gripTemplate.position.setY(PaperItems.gripTemplate.bounds.height/2 + 177);
		scope.loaded = true;
		//PaperItems.gripOutline.bounds.setCenter(view.center + new Point(2,31));
	});
	PaperLayers.product.importSVG('GripColor.svg',function(a){
	...
	PaperLayers.product.importSVG('GripOutline.svg',function(a){
...
PaperFunctions.resetBounds = function(){
	PaperLayers.overlay.activate();
	var productBounds = scope.printableSize;
	var boundsRect = new Rectangle(new Point(productBounds.originX, productBounds.originY), new Point(productBounds.originX + productBounds.width, productBounds.originY + productBounds.height));
	if (PaperItems.bounds){
		PaperItems.bounds.remove();
	}
	PaperItems.bounds = new Path.Rectangle(boundsRect);
	PaperItems.bounds.strokeColor = '#ff0000';
	PaperItems.bounds.strokeWidth = 3;
};
